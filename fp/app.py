...
# rota onde mostrará as contas pendentes do usuário
@app.route("/bills")
@login_required
def bills():
        ...
        # passa os valores de cada coluna do banco de dados e o preço total das contas para o html bills.html
        return render_template("bills.html", bills_rows=bills_rows, total=total)    

# adiciona uma conta (coloca na tabela de contas pendentes)
    ...
    if request.method == "POST":
        ...
        # If the date is valid insert into the table and update the bill remaining days to expire
            ...
            db.execute("INSERT INTO bills (id, name, price, validade, especificação, type) VALUES (:id , :name, :price, :validade, :especificação, :type)",
                        ...
                        type="topay")
            # insere os dias restantes na conta que acabou de ser inserida
...
# paga a conta do usuário(remove da tabela de contas pendentes e adiciona na tabela de contas pagas)
@app.route("/paybill", methods=["GET", "POST"])
@login_required
def paybill():
    if request.method == "POST":

        # seleciona os dados da conta que o usuário deseja pagar até linha 159
        name = request.form.get("name")
        ...
        # define hoje como o dia do pagamento
        payday = dtoday.strftime("%d/%m/%Y")

        # insere os dados da conta na tabela de contas pagas
        ...
        # deleta os dados da conta que foi paga da tabela de contas
...
# gera a pagina com as contas pagas pelo usuário
@app.route("/paidbills")
@login_required
def paidbills():
